...
DIE_COLORSCALE = [[0, "#f0f0f0"], [0.5, "#4c72b0"], [0.5, "#c44e52"], [1, "#c44e52"]]

PROCESS_MAP = {
    "Center": ("챔버", "RF 동작 불규칙, 유체 흐름 불균일", "챔버 정비 점검"),
    "Donut": ("세정", "포토레지스트 잔류물 미제거", "세정 공정 재점검"),
    "Edge-Ring": ("RTP", "온도 조절 이상, 저온 공정 편차", "RTP 온도 프로파일 조정"),
    "Edge-Loc": ("어닐링 / 로드락", "온도 불균일, 밸브 오염", "열공정 튜닝"),
    "Loc": ("국소 설비", "특정 위치 반복 결함", "해당 설비 점검"),
    "Scratch": ("핸들링", "기계적 접촉, 이송 장비", "이송 장비 점검"),
    "Random": ("파티클", "오염, 환경 요인", "클린룸 환경 점검"),
    "Near-full": ("전면 이상", "공정 전체 실패", "공정 전체 재검토"),
}
